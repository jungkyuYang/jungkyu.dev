import { getRecentUserActivity } from '@/shared/api/data';
...
export const RecentActivity = async ({ username }) => {
  ...
  const activitySummary = recentUserActivity.reduce((acc, activity) => {
    if (activity.type === 'PushEvent') {
      ...
      acc.prsMerged +=
        activity.payload.action === 'closed' && activity.payload.pull_request.merged ? 1 : 0;
    ...
    <div>
      <span className="text-sm">
        {activitySummaryString &&
          'In last 90 days on GitHub  I ' + activitySummaryString + ' in public repositories.'}
      </span>
